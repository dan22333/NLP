...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        # input validations:
        if len(sentence) != len(labels) or len(sentence) == 0:
            continue

        # add padding to the sentence
        sentence = [start] * window_size + sentence + [end] * window_size
        windowed_sentence = [([feature for word in sentence[i:(i + window_size * 2) + 1] for feature in word],\
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        # shape result is (None, n_window_features, embedding_size)
        embeddings_lookup = tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings), self.input_placeholder)
        embeddings = tf.reshape(embeddings_lookup, (-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        # Good link about dropout - https://medium.com/@amarbudhiraja/https-medium-com-amarbudhiraja-learning-less-to-learn-better-dropout-in-deep-machine-learning-74334da4bfc5
    ...
    def add_loss_op(self, pred):
        ...
        loss_flat = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=self.labels_placeholder)
