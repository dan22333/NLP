...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        delta = max_length - len(sentence)        
        new_sentence = (sentence             + [zero_vector] * delta) [:max_length]
        new_labels   = (labels               + [zero_label]  * delta) [:max_length]
        mask         = ([True]*len(sentence) + [False]       * delta) [:max_length]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings_lookup = tf.nn.embedding_lookup(embeddings_matrix, self.input_placeholder)
        embeddings = tf.reshape(embeddings_lookup, [-1, self.max_length, self.config.n_features * self.config.embed_size])                
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op_gru(self, x, dropout_rate):
        ...
        o, h   = tf.nn.dynamic_rnn(cell, x, initial_state = cell.zero_state(tf.shape(x)[0], tf.float32))                                
        o_drop = tf.nn.dropout(o, dropout_rate)        
        preds  = tf.layers.dense(o_drop, self.config.n_classes, kernel_initializer=tf.contrib.layers.xavier_initializer())
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        Returns:
            pred: A tensor of shape (batch_size, max_length, n_classes)
        """
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        batch_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss       = tf.reduce_mean(tf.boolean_mask(batch_loss, self.mask_placeholder))
        ### END YOUR CODE
        return loss
    ...
    def add_summary_op(self, pred, loss):
        ...
        #entropy    = -tf.tensordot(self.probs, tf.log(tf.clip_by_value(self.probs, 10^-3, 1)), [2, 2])        
        entropy = self.probs * tf.log(tf.clip_by_value(self.probs, 10^-3, 1))
        entropy = -tf.reduce_sum(entropy, axis=2)
        records.append(tf.summary.scalar('loss', loss))
        records.append(tf.summary.histogram('pred', tf.boolean_mask(pred, self.mask_placeholder)))
        records.append(tf.summary.scalar('entropy', tf.reduce_mean(tf.boolean_mask(entropy, self.mask_placeholder))))
        ### END YOUR CODE
