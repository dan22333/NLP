...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        sentence_ex = window_size * [start] + sentence + window_size * [end]
        for i in range(len(labels)):
            sentence_w = sentence_ex[i : i + 2*window_size + 1]
            window = [f for w in sentence_w for f in w]
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable('b1', shape=[self.config.hidden_size], initializer=tf.constant_initializer(0))
    ...
    def add_loss_op(self, pred):
        ...
        batch_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
